...
public class CreateUserHandlerTests
{
    ...
    public async Task Handle_ValidRequest_ReturnsSuccessResponse()
    {
        ...
        var user = new User
        {
            ...
            Role = command.Role,
            Name = new PersonNameValue(command.Name.FirstName, command.Name.LastName),
        ...
        new GeoLocationValue(command.Address.GeoLocation.Latitude, command.Address.GeoLocation.Longitude))
        };
    ...
    public async Task Handle_ValidRequest_MapsCommandToUser()
    {
        ...
        _mapper.Received(1).Map<User>(Arg.Is<CreateUserCommand>(c =>
            ...
            c.Name.LastName == command.Name.LastName &&
            c.Address.Street == command.Address.Street &&
            c.Address.Number == command.Address.Number &&
            c.Address.City == command.Address.City &&
            c.Address.State == command.Address.State &&
            c.Address.ZipCode == command.Address.ZipCode &&
            c.Address.GeoLocation.Latitude == command.Address.GeoLocation.Latitude &&
            c.Address.GeoLocation.Longitude == command.Address.GeoLocation.Longitude));
    }
}
